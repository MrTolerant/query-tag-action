...
try {
  ...
  var cmd =
    `${unshallowCmd} git describe --tags ${abbrevOption} ${includeOption} ${excludeOption} ${commitIshOption}`
      .replace(/[ ]+/, " ")
      .trim();
  ...
  exec(cmd, (err, tag, stderr) => {
    ...
    if (increment) {
      ...
      console.log("splitedTag:", splitedTag);
      const tagLength = splitedTag.length;
      console.log("tagLength:", tagLength);
      splitedTag[tagLength - 1] = +splitedTag[tagLength - 1]  + 1;
